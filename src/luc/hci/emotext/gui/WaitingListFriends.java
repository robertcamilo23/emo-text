...
public class WaitingListFriends extends ListActivity
{
	...
	public boolean onMenuItemSelected( int featureId, MenuItem item )
	{
		...
		case APPROVE_SELECTED_FRIENDS_ID:
		{
			...
			Thread thread = new Thread( )
			{
				...
				public void run( )
				{
					if ( approvedFriendNames.length( ) > 0 || discardedFriendNames.length( ) > 0 )
					{
	...
	private ServiceConnection mConnection = new ServiceConnection( )
	{
		...
		{
			imService = null;
			Toast.makeText( WaitingListFriends.this, R.string.local_service_stopped, Toast.LENGTH_SHORT ).show( );
		}
	};
}
